object pagibigsavings: Tpagibigsavings
  Left = 0
  Top = 0
  BorderStyle = bsDialog
  Caption = 'Pag-ibig Savings'
  ClientHeight = 548
  ClientWidth = 902
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  PixelsPerInch = 96
  TextHeight = 13
  object AdvPanel4: TAdvPanel
    Left = 0
    Top = 42
    Width = 902
    Height = 63
    Align = alTop
    BevelOuter = bvNone
    Color = 16645114
    Font.Charset = DEFAULT_CHARSET
    Font.Color = 7485192
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    UseDockManager = True
    Version = '2.3.0.0'
    BorderColor = 16765615
    Caption.Color = 16575452
    Caption.ColorTo = 16571329
    Caption.Font.Charset = DEFAULT_CHARSET
    Caption.Font.Color = clBlack
    Caption.Font.Height = -11
    Caption.Font.Name = 'Tahoma'
    Caption.Font.Style = []
    Caption.GradientDirection = gdVertical
    Caption.Indent = 2
    Caption.ShadeLight = 255
    CollapsColor = clNone
    CollapsDelay = 0
    ColorTo = 16643051
    ShadowColor = clBlack
    ShadowOffset = 0
    StatusBar.BorderColor = 13542013
    StatusBar.BorderStyle = bsSingle
    StatusBar.Font.Charset = DEFAULT_CHARSET
    StatusBar.Font.Color = 7485192
    StatusBar.Font.Height = -11
    StatusBar.Font.Name = 'Tahoma'
    StatusBar.Font.Style = []
    StatusBar.Color = 16575452
    StatusBar.ColorTo = 16571329
    StatusBar.GradientDirection = gdVertical
    Text = ''
    FullHeight = 200
    object AdvSmoothButton15: TAdvSmoothButton
      Left = 353
      Top = 15
      Width = 99
      Height = 35
      Appearance.PictureAlignment = taCenter
      Appearance.Font.Charset = DEFAULT_CHARSET
      Appearance.Font.Color = clBlack
      Appearance.Font.Height = -11
      Appearance.Font.Name = 'Tahoma'
      Appearance.Font.Style = [fsBold]
      Appearance.Spacing = 1
      Appearance.Rounding = 2
      Status.Caption = '0'
      Status.Appearance.Fill.Color = clRed
      Status.Appearance.Fill.ColorMirror = clNone
      Status.Appearance.Fill.ColorMirrorTo = clNone
      Status.Appearance.Fill.GradientType = gtSolid
      Status.Appearance.Fill.GradientMirrorType = gtSolid
      Status.Appearance.Fill.BorderColor = clGray
      Status.Appearance.Fill.Rounding = 0
      Status.Appearance.Fill.ShadowOffset = 0
      Status.Appearance.Fill.Glow = gmNone
      Status.Appearance.Font.Charset = DEFAULT_CHARSET
      Status.Appearance.Font.Color = clWhite
      Status.Appearance.Font.Height = -11
      Status.Appearance.Font.Name = 'Tahoma'
      Status.Appearance.Font.Style = []
      BevelColor = clBlack
      Caption = 'Close'
      Color = 16777088
      ParentFont = False
      Picture.Data = {
        
        A551B35F0000000049454E44AE426082}
      DisabledColor = 8404992
      TabOrder = 0
      ShowFocus = False
      Version = '2.0.1.0'
      OnClick = AdvSmoothButton15Click
    end
    object AdvSmoothButton14: TAdvSmoothButton
      Left = 248
      Top = 15
      Width = 99
      Height = 35
      Appearance.PictureAlignment = taCenter
      Appearance.Font.Charset = DEFAULT_CHARSET
      Appearance.Font.Color = clBlack
      Appearance.Font.Height = -11
      Appearance.Font.Name = 'Tahoma'
      Appearance.Font.Style = [fsBold]
      Appearance.Spacing = 1
      Appearance.Rounding = 2
      Status.Caption = '0'
      Status.Appearance.Fill.Color = clRed
      Status.Appearance.Fill.ColorMirror = clNone
      Status.Appearance.Fill.ColorMirrorTo = clNone
      Status.Appearance.Fill.GradientType = gtSolid
      Status.Appearance.Fill.GradientMirrorType = gtSolid
      Status.Appearance.Fill.BorderColor = clGray
      Status.Appearance.Fill.Rounding = 0
      Status.Appearance.Fill.ShadowOffset = 0
      Status.Appearance.Fill.Glow = gmNone
      Status.Appearance.Font.Charset = DEFAULT_CHARSET
      Status.Appearance.Font.Color = clWhite
      Status.Appearance.Font.Height = -11
      Status.Appearance.Font.Name = 'Tahoma'
      Status.Appearance.Font.Style = []
      BevelColor = clBlack
      Caption = 'Delete'
      Color = 16777088
      ParentFont = False
      Picture.Data = {
        
        D7849FE4BDE7510000000049454E44AE426082}
      DisabledColor = 8404992
      TabOrder = 1
      ShowFocus = False
      Version = '2.0.1.0'
    end
    object AdvSmoothButton13: TAdvSmoothButton
      Left = 144
      Top = 15
      Width = 99
      Height = 35
      Appearance.PictureAlignment = taCenter
      Appearance.Font.Charset = DEFAULT_CHARSET
      Appearance.Font.Color = clBlack
      Appearance.Font.Height = -11
      Appearance.Font.Name = 'Tahoma'
      Appearance.Font.Style = [fsBold]
      Appearance.Spacing = 1
      Appearance.Rounding = 2
      Status.Caption = '0'
      Status.Appearance.Fill.Color = clRed
      Status.Appearance.Fill.ColorMirror = clNone
      Status.Appearance.Fill.ColorMirrorTo = clNone
      Status.Appearance.Fill.GradientType = gtSolid
      Status.Appearance.Fill.GradientMirrorType = gtSolid
      Status.Appearance.Fill.BorderColor = clGray
      Status.Appearance.Fill.Rounding = 0
      Status.Appearance.Fill.ShadowOffset = 0
      Status.Appearance.Fill.Glow = gmNone
      Status.Appearance.Font.Charset = DEFAULT_CHARSET
      Status.Appearance.Font.Color = clWhite
      Status.Appearance.Font.Height = -11
      Status.Appearance.Font.Name = 'Tahoma'
      Status.Appearance.Font.Style = []
      BevelColor = clBlack
      Caption = 'Update '
      Color = 16777088
      ParentFont = False
      Picture.Data = {
        
        3CBF00D290849D860F104B0000000049454E44AE426082}
      DisabledColor = 8404992
      TabOrder = 2
      ShowFocus = False
      Version = '2.0.1.0'
      OnClick = AdvSmoothButton13Click
    end
    object AdvSmoothButton12: TAdvSmoothButton
      Left = 39
      Top = 15
      Width = 99
      Height = 35
      Appearance.PictureAlignment = taCenter
      Appearance.Font.Charset = DEFAULT_CHARSET
      Appearance.Font.Color = clBlack
      Appearance.Font.Height = -11
      Appearance.Font.Name = 'Tahoma'
      Appearance.Font.Style = [fsBold]
      Appearance.Spacing = 1
      Appearance.Rounding = 2
      Status.Caption = '0'
      Status.Appearance.Fill.Color = clRed
      Status.Appearance.Fill.ColorMirror = clNone
      Status.Appearance.Fill.ColorMirrorTo = clNone
      Status.Appearance.Fill.GradientType = gtSolid
      Status.Appearance.Fill.GradientMirrorType = gtSolid
      Status.Appearance.Fill.BorderColor = clGray
      Status.Appearance.Fill.Rounding = 0
      Status.Appearance.Fill.ShadowOffset = 0
      Status.Appearance.Fill.Glow = gmNone
      Status.Appearance.Font.Charset = DEFAULT_CHARSET
      Status.Appearance.Font.Color = clWhite
      Status.Appearance.Font.Height = -11
      Status.Appearance.Font.Name = 'Tahoma'
      Status.Appearance.Font.Style = []
      BevelColor = clBlack
      Caption = 'Add '
      Color = 16777088
      ParentFont = False
      Picture.Data = {
        }
      DisabledColor = 8404992
      TabOrder = 3
      ShowFocus = False
      Version = '2.0.1.0'
      OnClick = AdvSmoothButton12Click
    end
  end
  object AdvPanel3: TAdvPanel
    Left = 0
    Top = 105
    Width = 902
    Height = 64
    Align = alTop
    BevelOuter = bvNone
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    UseDockManager = True
    Version = '2.3.0.0'
    Caption.Color = clHighlight
    Caption.ColorTo = clNone
    Caption.Font.Charset = DEFAULT_CHARSET
    Caption.Font.Color = clWindowText
    Caption.Font.Height = -11
    Caption.Font.Name = 'Tahoma'
    Caption.Font.Style = []
    StatusBar.Font.Charset = DEFAULT_CHARSET
    StatusBar.Font.Color = clWindowText
    StatusBar.Font.Height = -11
    StatusBar.Font.Name = 'Tahoma'
    StatusBar.Font.Style = []
    Text = ''
    FullHeight = 200
    object Label1: TLabel
      Left = 40
      Top = 9
      Width = 97
      Height = 14
      Caption = 'Search by Name'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object AdvEdit1: TAdvEdit
      Left = 40
      Top = 29
      Width = 224
      Height = 22
      EmptyTextStyle = []
      LabelFont.Charset = DEFAULT_CHARSET
      LabelFont.Color = clWindowText
      LabelFont.Height = -11
      LabelFont.Name = 'Tahoma'
      LabelFont.Style = []
      Lookup.Font.Charset = DEFAULT_CHARSET
      Lookup.Font.Color = clWindowText
      Lookup.Font.Height = -11
      Lookup.Font.Name = 'Arial'
      Lookup.Font.Style = []
      Lookup.Separator = ';'
      Color = clWindow
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      Text = ''
      Visible = True
      Version = '3.3.2.0'
    end
    object AdvGlowButton1: TAdvGlowButton
      Left = 266
      Top = 26
      Width = 62
      Height = 25
      Caption = 'Search'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      ImageIndex = 6
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      ParentFont = False
      TabOrder = 1
      OnClick = AdvGlowButton1Click
      Appearance.ColorChecked = 16111818
      Appearance.ColorCheckedTo = 16367008
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 16111818
      Appearance.ColorDownTo = 16367008
      Appearance.ColorHot = 16117985
      Appearance.ColorHotTo = 16372402
      Appearance.ColorMirrorHot = 16107693
      Appearance.ColorMirrorHotTo = 16775412
      Appearance.ColorMirrorDown = 16102556
      Appearance.ColorMirrorDownTo = 16768988
      Appearance.ColorMirrorChecked = 16102556
      Appearance.ColorMirrorCheckedTo = 16768988
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
    end
  end
  object CRDBGrid1: TCRDBGrid
    Left = 0
    Top = 169
    Width = 902
    Height = 303
    OptionsEx = [dgeEnableSort, dgeLocalFilter, dgeLocalSorting, dgeSummary]
    Align = alClient
    DataSource = savingDS
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
    ParentFont = False
    ReadOnly = True
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -12
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'empnumber'
        Title.Caption = 'Emp.No.'
        Width = 68
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'name'
        Title.Caption = 'Name'
        Width = 215
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'departmentname'
        Title.Caption = 'Department'
        Width = 176
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'empstatus'
        Title.Caption = 'Status'
        Width = 84
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'amount'
        Title.Alignment = taRightJustify
        Title.Caption = 'Amount'
        Width = 131
        Visible = True
        FloatFormat = ffNumber
        FloatPrecision = 2
      end>
  end
  object AdvPanel2: TAdvPanel
    Left = 0
    Top = 472
    Width = 902
    Height = 76
    Align = alBottom
    BevelOuter = bvNone
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 3
    UseDockManager = True
    Version = '2.3.0.0'
    Caption.Color = clHighlight
    Caption.ColorTo = clNone
    Caption.Font.Charset = DEFAULT_CHARSET
    Caption.Font.Color = clWindowText
    Caption.Font.Height = -11
    Caption.Font.Name = 'Tahoma'
    Caption.Font.Style = []
    Caption.Height = 144
    StatusBar.Font.Charset = DEFAULT_CHARSET
    StatusBar.Font.Color = clWindowText
    StatusBar.Font.Height = -11
    StatusBar.Font.Name = 'Tahoma'
    StatusBar.Font.Style = []
    Text = ''
    FullHeight = 200
    object NxLinkLabel1: TNxLinkLabel
      Left = 66
      Top = 10
      Width = 111
      Height = 20
      Caption = 'Employee Details'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 9192960
      Font.Height = -15
      Font.Name = 'Tahoma'
      Font.Style = []
      HoverColor = 9192960
      ParentFont = False
      TextDistance = 2
      Transparent = True
      VertSpacing = 2
      OnClick = NxLinkLabel1Click
    end
    object NxLinkLabel2: TNxLinkLabel
      Left = 66
      Top = 36
      Width = 114
      Height = 20
      Caption = 'Paymonth Details'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 9192960
      Font.Height = -15
      Font.Name = 'Tahoma'
      Font.Style = []
      HoverColor = 9192960
      ParentFont = False
      TextDistance = 2
      Transparent = True
      VertSpacing = 2
      OnClick = NxLinkLabel2Click
    end
  end
  object AdvPanel1: TAdvPanel
    Left = 0
    Top = 0
    Width = 902
    Height = 42
    Align = alTop
    BevelOuter = bvNone
    Color = 16744576
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 4
    UseDockManager = True
    Version = '2.3.0.0'
    BorderColor = 7171437
    Caption.Color = 12566463
    Caption.ColorTo = 9539985
    Caption.Font.Charset = DEFAULT_CHARSET
    Caption.Font.Color = clWindowText
    Caption.Font.Height = -11
    Caption.Font.Name = 'Tahoma'
    Caption.Font.Style = []
    Caption.GradientDirection = gdVertical
    Caption.Indent = 2
    Caption.ShadeLight = 255
    CollapsColor = 8404992
    CollapsDelay = 2
    ColorTo = 8404992
    ColorMirror = 8404992
    ColorMirrorTo = 8404992
    ShadowColor = clBlack
    ShadowOffset = 0
    StatusBar.BorderColor = 12566463
    StatusBar.BorderStyle = bsSingle
    StatusBar.Font.Charset = DEFAULT_CHARSET
    StatusBar.Font.Color = 14145494
    StatusBar.Font.Height = -11
    StatusBar.Font.Name = 'Tahoma'
    StatusBar.Font.Style = []
    StatusBar.Color = 12566463
    StatusBar.ColorTo = 9539985
    StatusBar.GradientDirection = gdVertical
    Text = ''
    FullHeight = 200
    object AdvSmoothLabel1: TAdvSmoothLabel
      Left = -6
      Top = 3
      Width = 280
      Height = 33
      Fill.ColorMirror = clNone
      Fill.ColorMirrorTo = clNone
      Fill.GradientType = gtVertical
      Fill.GradientMirrorType = gtSolid
      Fill.BorderColor = clNone
      Fill.Rounding = 0
      Fill.ShadowOffset = 0
      Fill.Glow = gmNone
      Caption.Text = '    Pag-ibi Savings  List  '
      Caption.Location = plBottomLeft
      Caption.Font.Charset = DEFAULT_CHARSET
      Caption.Font.Color = clWindowText
      Caption.Font.Height = -19
      Caption.Font.Name = 'Arial Black'
      Caption.Font.Style = [fsBold]
      Caption.ColorStart = 8454016
      Caption.ColorEnd = clWhite
      CaptionShadow.Text = '    Pag-ibi Savings  List  '
      CaptionShadow.Font.Charset = DEFAULT_CHARSET
      CaptionShadow.Font.Color = clWindowText
      CaptionShadow.Font.Height = -27
      CaptionShadow.Font.Name = 'Tahoma'
      CaptionShadow.Font.Style = []
      Version = '1.6.0.2'
    end
  end
  object NxHeaderPanel2: TNxHeaderPanel
    Left = 266
    Top = 192
    Width = 395
    Height = 241
    Caption = 'Add Employee Pag-ibig Savings'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    HeaderFont.Charset = DEFAULT_CHARSET
    HeaderFont.Color = clWindowText
    HeaderFont.Height = -11
    HeaderFont.Name = 'Tahoma'
    HeaderFont.Style = [fsBold]
    HeaderStyle = psWindowsLive
    PanelStyle = ptGradient
    ParentFont = False
    ParentHeaderFont = False
    TabOrder = 5
    Visible = False
    FullWidth = 395
    object Label11: TLabel
      Left = 59
      Top = 61
      Width = 55
      Height = 13
      Alignment = taRightJustify
      Caption = 'Employee'
    end
    object Label2: TLabel
      Left = 70
      Top = 87
      Width = 44
      Height = 13
      Alignment = taRightJustify
      Caption = 'Emp.No.'
    end
    object Label3: TLabel
      Left = 45
      Top = 116
      Width = 69
      Height = 13
      Alignment = taRightJustify
      Caption = 'Department'
    end
    object Label4: TLabel
      Left = 69
      Top = 142
      Width = 45
      Height = 13
      Alignment = taRightJustify
      Caption = 'Amount'
    end
    object name_tf: TAdvLUEdit
      Left = 120
      Top = 56
      Width = 200
      Height = 21
      EmptyTextStyle = []
      LabelFont.Charset = DEFAULT_CHARSET
      LabelFont.Color = clWindowText
      LabelFont.Height = -11
      LabelFont.Name = 'Tahoma'
      LabelFont.Style = []
      Lookup.Font.Charset = DEFAULT_CHARSET
      Lookup.Font.Color = clWindowText
      Lookup.Font.Height = -11
      Lookup.Font.Name = 'Arial'
      Lookup.Font.Style = []
      Lookup.Separator = ';'
      Color = clWindow
      ReadOnly = True
      TabOrder = 0
      Text = ''
      Visible = True
      Version = '1.4.1.0'
      AutoHistory = False
      AutoSynchronize = False
      FileLookup = False
      LookupPersist.Enable = False
      LookupPersist.Location = plInifile
      LookupPersist.Count = 0
      LookupPersist.MaxCount = False
      MatchCase = False
    end
    object AdvSmoothButton9: TAdvSmoothButton
      Left = 122
      Top = 183
      Width = 70
      Height = 31
      Appearance.PictureAlignment = taCenter
      Appearance.Font.Charset = DEFAULT_CHARSET
      Appearance.Font.Color = clBlack
      Appearance.Font.Height = -11
      Appearance.Font.Name = 'Tahoma'
      Appearance.Font.Style = [fsBold]
      Appearance.Spacing = 1
      Appearance.Rounding = 2
      Status.Caption = '0'
      Status.Appearance.Fill.Color = clRed
      Status.Appearance.Fill.ColorMirror = clNone
      Status.Appearance.Fill.ColorMirrorTo = clNone
      Status.Appearance.Fill.GradientType = gtSolid
      Status.Appearance.Fill.GradientMirrorType = gtSolid
      Status.Appearance.Fill.BorderColor = clGray
      Status.Appearance.Fill.Rounding = 0
      Status.Appearance.Fill.ShadowOffset = 0
      Status.Appearance.Fill.Glow = gmNone
      Status.Appearance.Font.Charset = DEFAULT_CHARSET
      Status.Appearance.Font.Color = clWhite
      Status.Appearance.Font.Height = -11
      Status.Appearance.Font.Name = 'Tahoma'
      Status.Appearance.Font.Style = []
      BevelColor = clBlack
      Caption = 'Save'
      Color = 16777088
      ParentFont = False
      Picture.Data = {
        }
      DisabledColor = 8404992
      TabOrder = 1
      ShowFocus = False
      Version = '2.0.1.0'
      OnClick = AdvSmoothButton9Click
    end
    object AdvSmoothButton1: TAdvSmoothButton
      Left = 198
      Top = 183
      Width = 66
      Height = 31
      Appearance.PictureAlignment = taCenter
      Appearance.Font.Charset = DEFAULT_CHARSET
      Appearance.Font.Color = clBlack
      Appearance.Font.Height = -11
      Appearance.Font.Name = 'Tahoma'
      Appearance.Font.Style = [fsBold]
      Appearance.Spacing = 1
      Appearance.Rounding = 2
      Status.Caption = '0'
      Status.Appearance.Fill.Color = clRed
      Status.Appearance.Fill.ColorMirror = clNone
      Status.Appearance.Fill.ColorMirrorTo = clNone
      Status.Appearance.Fill.GradientType = gtSolid
      Status.Appearance.Fill.GradientMirrorType = gtSolid
      Status.Appearance.Fill.BorderColor = clGray
      Status.Appearance.Fill.Rounding = 0
      Status.Appearance.Fill.ShadowOffset = 0
      Status.Appearance.Fill.Glow = gmNone
      Status.Appearance.Font.Charset = DEFAULT_CHARSET
      Status.Appearance.Font.Color = clWhite
      Status.Appearance.Font.Height = -11
      Status.Appearance.Font.Name = 'Tahoma'
      Status.Appearance.Font.Style = []
      BevelColor = clBlack
      Caption = 'Cancel'
      Color = 16777088
      ParentFont = False
      Picture.Data = {
        
        0300FD675A136B6939390000000049454E44AE426082}
      DisabledColor = 8404992
      TabOrder = 2
      ShowFocus = False
      Version = '2.0.1.0'
      OnClick = AdvSmoothButton1Click
    end
    object AdvGlowButton2: TAdvGlowButton
      Left = 326
      Top = 52
      Width = 38
      Height = 25
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      ImageIndex = 6
      Images = ImageList1
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      ParentFont = False
      TabOrder = 3
      OnClick = AdvGlowButton2Click
      Appearance.ColorChecked = 16111818
      Appearance.ColorCheckedTo = 16367008
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 16111818
      Appearance.ColorDownTo = 16367008
      Appearance.ColorHot = 16117985
      Appearance.ColorHotTo = 16372402
      Appearance.ColorMirrorHot = 16107693
      Appearance.ColorMirrorHotTo = 16775412
      Appearance.ColorMirrorDown = 16102556
      Appearance.ColorMirrorDownTo = 16768988
      Appearance.ColorMirrorChecked = 16102556
      Appearance.ColorMirrorCheckedTo = 16768988
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
    end
    object empnumber_tf: TAdvLUEdit
      Left = 120
      Top = 83
      Width = 200
      Height = 21
      EmptyTextStyle = []
      LabelFont.Charset = DEFAULT_CHARSET
      LabelFont.Color = clWindowText
      LabelFont.Height = -11
      LabelFont.Name = 'Tahoma'
      LabelFont.Style = []
      Lookup.Font.Charset = DEFAULT_CHARSET
      Lookup.Font.Color = clWindowText
      Lookup.Font.Height = -11
      Lookup.Font.Name = 'Arial'
      Lookup.Font.Style = []
      Lookup.Separator = ';'
      Color = clWindow
      ReadOnly = True
      TabOrder = 4
      Text = ''
      Visible = True
      Version = '1.4.1.0'
      AutoHistory = False
      AutoSynchronize = False
      FileLookup = False
      LookupPersist.Enable = False
      LookupPersist.Location = plInifile
      LookupPersist.Count = 0
      LookupPersist.MaxCount = False
      MatchCase = False
    end
    object department_tf: TAdvLUEdit
      Left = 120
      Top = 110
      Width = 200
      Height = 21
      EmptyTextStyle = []
      LabelFont.Charset = DEFAULT_CHARSET
      LabelFont.Color = clWindowText
      LabelFont.Height = -11
      LabelFont.Name = 'Tahoma'
      LabelFont.Style = []
      Lookup.Font.Charset = DEFAULT_CHARSET
      Lookup.Font.Color = clWindowText
      Lookup.Font.Height = -11
      Lookup.Font.Name = 'Arial'
      Lookup.Font.Style = []
      Lookup.Separator = ';'
      Color = clWindow
      ReadOnly = True
      TabOrder = 5
      Text = ''
      Visible = True
      Version = '1.4.1.0'
      AutoHistory = False
      AutoSynchronize = False
      FileLookup = False
      LookupPersist.Enable = False
      LookupPersist.Location = plInifile
      LookupPersist.Count = 0
      LookupPersist.MaxCount = False
      MatchCase = False
    end
    object amount_tf: TAdvLUEdit
      Left = 120
      Top = 137
      Width = 200
      Height = 21
      EmptyTextStyle = []
      LabelFont.Charset = DEFAULT_CHARSET
      LabelFont.Color = clWindowText
      LabelFont.Height = -11
      LabelFont.Name = 'Tahoma'
      LabelFont.Style = []
      Lookup.Font.Charset = DEFAULT_CHARSET
      Lookup.Font.Color = clWindowText
      Lookup.Font.Height = -11
      Lookup.Font.Name = 'Arial'
      Lookup.Font.Style = []
      Lookup.Separator = ';'
      Color = clWindow
      TabOrder = 6
      Text = ''
      Visible = True
      Version = '1.4.1.0'
      AutoHistory = False
      AutoSynchronize = False
      FileLookup = False
      LookupPersist.Enable = False
      LookupPersist.Location = plInifile
      LookupPersist.Count = 0
      LookupPersist.MaxCount = False
      MatchCase = False
    end
    object idemployee_tf: TAdvLUEdit
      Left = 120
      Top = 29
      Width = 200
      Height = 21
      EmptyTextStyle = []
      LabelFont.Charset = DEFAULT_CHARSET
      LabelFont.Color = clWindowText
      LabelFont.Height = -11
      LabelFont.Name = 'Tahoma'
      LabelFont.Style = []
      Lookup.Font.Charset = DEFAULT_CHARSET
      Lookup.Font.Color = clWindowText
      Lookup.Font.Height = -11
      Lookup.Font.Name = 'Arial'
      Lookup.Font.Style = []
      Lookup.Separator = ';'
      Color = clWindow
      ReadOnly = True
      TabOrder = 7
      Text = ''
      Visible = False
      Version = '1.4.1.0'
      AutoHistory = False
      AutoSynchronize = False
      FileLookup = False
      LookupPersist.Enable = False
      LookupPersist.Location = plInifile
      LookupPersist.Count = 0
      LookupPersist.MaxCount = False
      MatchCase = False
    end
    object empstatus_tf: TAdvLUEdit
      Left = 122
      Top = 268
      Width = 200
      Height = 21
      EmptyTextStyle = []
      LabelFont.Charset = DEFAULT_CHARSET
      LabelFont.Color = clWindowText
      LabelFont.Height = -11
      LabelFont.Name = 'Tahoma'
      LabelFont.Style = []
      Lookup.Font.Charset = DEFAULT_CHARSET
      Lookup.Font.Color = clWindowText
      Lookup.Font.Height = -11
      Lookup.Font.Name = 'Arial'
      Lookup.Font.Style = []
      Lookup.Separator = ';'
      Color = clWindow
      ReadOnly = True
      TabOrder = 8
      Text = ''
      Visible = False
      Version = '1.4.1.0'
      AutoHistory = False
      AutoSynchronize = False
      FileLookup = False
      LookupPersist.Enable = False
      LookupPersist.Location = plInifile
      LookupPersist.Count = 0
      LookupPersist.MaxCount = False
      MatchCase = False
    end
  end
  object NxHeaderPanel1: TNxHeaderPanel
    Left = 168
    Top = 219
    Width = 336
    Height = 188
    Caption = 'Select Paymonth to Preview'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    HeaderFont.Charset = DEFAULT_CHARSET
    HeaderFont.Color = clWindowText
    HeaderFont.Height = -11
    HeaderFont.Name = 'Tahoma'
    HeaderFont.Style = [fsBold]
    HeaderStyle = psWindowsLive
    PanelStyle = ptGradient
    ParentFont = False
    ParentHeaderFont = False
    TabOrder = 6
    Visible = False
    FullWidth = 336
    object Label5: TLabel
      Left = 32
      Top = 54
      Width = 58
      Height = 13
      Alignment = taRightJustify
      Caption = 'Paymonth'
    end
    object Label6: TLabel
      Left = 53
      Top = 87
      Width = 37
      Height = 13
      Alignment = taRightJustify
      Caption = 'Status'
    end
    object AdvLUEdit6: TAdvLUEdit
      Left = 122
      Top = 268
      Width = 200
      Height = 21
      EmptyTextStyle = []
      LabelFont.Charset = DEFAULT_CHARSET
      LabelFont.Color = clWindowText
      LabelFont.Height = -11
      LabelFont.Name = 'Tahoma'
      LabelFont.Style = []
      Lookup.Font.Charset = DEFAULT_CHARSET
      Lookup.Font.Color = clWindowText
      Lookup.Font.Height = -11
      Lookup.Font.Name = 'Arial'
      Lookup.Font.Style = []
      Lookup.Separator = ';'
      Color = clWindow
      ReadOnly = True
      TabOrder = 0
      Text = ''
      Visible = False
      Version = '1.4.1.0'
      AutoHistory = False
      AutoSynchronize = False
      FileLookup = False
      LookupPersist.Enable = False
      LookupPersist.Location = plInifile
      LookupPersist.Count = 0
      LookupPersist.MaxCount = False
      MatchCase = False
    end
    object paymonth_cb: TNxComboBox
      Left = 96
      Top = 49
      Width = 217
      Height = 21
      Cursor = crArrow
      TabOrder = 1
      ReadOnly = True
      HideFocus = False
      Style = dsDropDownList
      AutoCompleteDelay = 0
    end
    object NxLinkLabel4: TNxLinkLabel
      Left = 105
      Top = 137
      Width = 88
      Height = 18
      Caption = 'Preview Details'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 9192960
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      HoverColor = 9192960
      ParentFont = False
      TextDistance = 2
      Transparent = True
      VertSpacing = 2
      OnClick = NxLinkLabel4Click
    end
    object NxLinkLabel5: TNxLinkLabel
      Left = 288
      Top = 5
      Width = 32
      Height = 18
      Caption = 'Close'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      HoverColor = clHotLight
      ParentFont = False
      TextDistance = 2
      Transparent = True
      VertSpacing = 2
      OnClick = NxLinkLabel5Click
    end
    object empstatus_cb: TNxComboBox
      Left = 96
      Top = 83
      Width = 217
      Height = 21
      Cursor = crArrow
      TabOrder = 4
      ReadOnly = True
      HideFocus = False
      Style = dsDropDownList
      AutoCompleteDelay = 0
      Items.Strings = (
        'REGULAR PAYROLL'
        'CASUAL PAYROLL')
    end
    object NxLinkLabel8: TNxLinkLabel
      Left = 221
      Top = 137
      Width = 85
      Height = 18
      Caption = 'Export to Excel'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 9192960
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      HoverColor = 9192960
      ParentFont = False
      TextDistance = 2
      Transparent = True
      VertSpacing = 2
      OnClick = NxLinkLabel8Click
    end
  end
  object NxHeaderPanel3: TNxHeaderPanel
    Left = 667
    Top = 219
    Width = 279
    Height = 166
    Caption = 'Set Period Covered'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    HeaderFont.Charset = DEFAULT_CHARSET
    HeaderFont.Color = clWindowText
    HeaderFont.Height = -11
    HeaderFont.Name = 'Tahoma'
    HeaderFont.Style = [fsBold]
    HeaderStyle = psWindowsLive
    PanelStyle = ptGradient
    ParentFont = False
    ParentHeaderFont = False
    TabOrder = 7
    Visible = False
    FullWidth = 279
    object Label7: TLabel
      Left = 61
      Top = 54
      Width = 29
      Height = 13
      Alignment = taRightJustify
      Caption = 'From'
    end
    object Label8: TLabel
      Left = 76
      Top = 82
      Width = 14
      Height = 13
      Alignment = taRightJustify
      Caption = 'To'
    end
    object AdvLUEdit1: TAdvLUEdit
      Left = 122
      Top = 268
      Width = 200
      Height = 21
      EmptyTextStyle = []
      LabelFont.Charset = DEFAULT_CHARSET
      LabelFont.Color = clWindowText
      LabelFont.Height = -11
      LabelFont.Name = 'Tahoma'
      LabelFont.Style = []
      Lookup.Font.Charset = DEFAULT_CHARSET
      Lookup.Font.Color = clWindowText
      Lookup.Font.Height = -11
      Lookup.Font.Name = 'Arial'
      Lookup.Font.Style = []
      Lookup.Separator = ';'
      Color = clWindow
      ReadOnly = True
      TabOrder = 0
      Text = ''
      Visible = False
      Version = '1.4.1.0'
      AutoHistory = False
      AutoSynchronize = False
      FileLookup = False
      LookupPersist.Enable = False
      LookupPersist.Location = plInifile
      LookupPersist.Count = 0
      LookupPersist.MaxCount = False
      MatchCase = False
    end
    object NxLinkLabel6: TNxLinkLabel
      Left = 96
      Top = 124
      Width = 71
      Height = 18
      Caption = 'View Details'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 9192960
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      HoverColor = 9192960
      ParentFont = False
      TextDistance = 2
      Transparent = True
      VertSpacing = 2
      OnClick = NxLinkLabel6Click
    end
    object NxLinkLabel7: TNxLinkLabel
      Left = 216
      Top = 118
      Width = 39
      Height = 18
      Caption = 'Cancel'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 9192960
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      HoverColor = 9192960
      ParentFont = False
      TextDistance = 2
      Transparent = True
      VertSpacing = 2
      OnClick = NxLinkLabel7Click
    end
    object dFrom: TNxDatePicker
      Left = 96
      Top = 46
      Width = 121
      Height = 21
      TabOrder = 3
      Text = '2/10/2020'
      HideFocus = False
      Date = 43871.000000000000000000
      NoneCaption = 'None'
      TodayCaption = 'Today'
    end
    object dTo: TNxDatePicker
      Left = 96
      Top = 76
      Width = 121
      Height = 21
      TabOrder = 4
      Text = '2/10/2020'
      HideFocus = False
      Date = 43871.000000000000000000
      NoneCaption = 'None'
      TodayCaption = 'Today'
    end
  end
  object ImageList1: TImageList
    Left = 547
    Top = 56
    Bitmap = {
      
      000000000000}
  end
  object savingQ: TMyQuery
    Connection = PayrollData.MyConnection1
    SQL.Strings = (
      'Select f.*'
      'from'
      '(SELECT '
      ' p.idemployee,'
      ' p.empnumber,'
      ' p.amount,'
      
        ' concat(e.firstname,'#39' '#39',e.middleinitial,'#39', '#39',e.lastname) as name' +
        ','
      ' e.departmentname,'
      ' case e.employeestatus'
      '   when '#39'C'#39' then '#39'Casual'#39
      '   when '#39'R'#39' then '#39'Regular'#39
      '   when '#39'B'#39' then '#39'Board'#39
      '   ELSE '#39#39
      ' END as empstatus'
      ''
      'FROM pagibigsavingsdebt p'
      'left join employee e on e.idemployee = p.idemployee'
      ') f'
      'where f.name like :name')
    Left = 200
    Top = 264
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'name'
        Value = nil
      end>
    object savingQidemployee: TIntegerField
      FieldName = 'idemployee'
    end
    object savingQempnumber: TStringField
      FieldName = 'empnumber'
      Size = 15
    end
    object savingQamount: TFloatField
      FieldName = 'amount'
      currency = True
    end
    object savingQname: TStringField
      FieldName = 'name'
      Size = 68
    end
    object savingQdepartmentname: TStringField
      FieldName = 'departmentname'
      Size = 40
    end
    object savingQempstatus: TStringField
      FieldName = 'empstatus'
      Size = 7
    end
  end
  object savingDS: TMyDataSource
    DataSet = savingQ
    Left = 192
    Top = 320
  end
  object insertsavingsQ: TMyQuery
    Connection = PayrollData.MyConnection1
    SQL.Strings = (
      'insert into pagibigsavingsdebt(idemployee,empnumber,amount,user)'
      'values(:idemployee,:empnumber,:amount,:user)')
    Left = 128
    Top = 256
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'idemployee'
        Value = nil
      end
      item
        DataType = ftUnknown
        Name = 'empnumber'
        Value = nil
      end
      item
        DataType = ftUnknown
        Name = 'amount'
        Value = nil
      end
      item
        DataType = ftUnknown
        Name = 'user'
        Value = nil
      end>
  end
  object updateSavingsQ: TMyQuery
    Connection = PayrollData.MyConnection1
    SQL.Strings = (
      'update pagibigsavingsdebt set'
      ' amount = :amount'
      'where idemployee = :idemployee '
      ' ')
    Left = 32
    Top = 248
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'amount'
        Value = nil
      end
      item
        DataType = ftUnknown
        Name = 'idemployee'
        Value = nil
      end>
  end
  object paymonthQ: TMyQuery
    Connection = PayrollData.MyConnection1
    SQL.Strings = (
      'SELECT '
      '  * '
      'FROM zanecopayroll.payrollmaster '
      'where paymonth > '#39'2019-01-31'#39' AND '
      
        '     (description = '#39'CASUAL PAYROLL'#39' OR description = '#39'REGULAR P' +
        'AYROLL'#39') AND '
      '     PAGIBIGsavings > 0   '
      'group by paymonth '
      'order by idpayrollmaster desc;')
    Left = 688
    Top = 112
    object paymonthQidpayrollmaster: TLongWordField
      FieldName = 'idpayrollmaster'
    end
    object paymonthQidemployee: TFloatField
      FieldName = 'idemployee'
    end
    object paymonthQpaymonth: TDateField
      FieldName = 'paymonth'
    end
    object paymonthQdescription: TStringField
      FieldName = 'description'
      Size = 50
    end
    object paymonthQempnumber: TStringField
      FieldName = 'empnumber'
      Size = 15
    end
    object paymonthQname: TStringField
      FieldName = 'name'
      Size = 50
    end
    object paymonthQdeptcode: TStringField
      FieldName = 'deptcode'
      Size = 30
    end
    object paymonthQbasicpay: TFloatField
      FieldName = 'basicpay'
    end
    object paymonthQDifferential: TFloatField
      FieldName = 'Differential'
    end
    object paymonthQUnpaidAmt: TFloatField
      FieldName = 'UnpaidAmt'
    end
    object paymonthQNumUnpaidDays: TFloatField
      FieldName = 'NumUnpaidDays'
    end
    object paymonthQUnpaidRate: TFloatField
      FieldName = 'UnpaidRate'
    end
    object paymonthQpremiumpay: TFloatField
      FieldName = 'premiumpay'
    end
    object paymonthQRegOtPay: TFloatField
      FieldName = 'RegOtPay'
    end
    object paymonthQSpcOTPay: TFloatField
      FieldName = 'SpcOTPay'
    end
    object paymonthQRestOtPay: TFloatField
      FieldName = 'RestOtPay'
    end
    object paymonthQRestOtExPay: TFloatField
      FieldName = 'RestOtExPay'
    end
    object paymonthQLegOTPay: TFloatField
      FieldName = 'LegOTPay'
    end
    object paymonthQLegOtExPay: TFloatField
      FieldName = 'LegOtExPay'
    end
    object paymonthQLegRestOtPay: TFloatField
      FieldName = 'LegRestOtPay'
    end
    object paymonthQSpcOtExPay: TFloatField
      FieldName = 'SpcOtExPay'
    end
    object paymonthQnsd: TFloatField
      FieldName = 'nsd'
    end
    object paymonthQundertime: TFloatField
      FieldName = 'undertime'
    end
    object paymonthQabsences: TFloatField
      FieldName = 'absences'
    end
    object paymonthQriceallowance: TFloatField
      FieldName = 'riceallowance'
    end
    object paymonthQhazard: TFloatField
      FieldName = 'hazard'
    end
    object paymonthQlighting: TFloatField
      FieldName = 'lighting'
    end
    object paymonthQMonth13: TFloatField
      FieldName = 'Month13'
    end
    object paymonthQBonus: TFloatField
      FieldName = 'Bonus'
    end
    object paymonthQOtherPay: TFloatField
      FieldName = 'OtherPay'
    end
    object paymonthQPAGIBIGCont: TFloatField
      FieldName = 'PAGIBIGCont'
    end
    object paymonthQSSSCont: TFloatField
      FieldName = 'SSSCont'
    end
    object paymonthQPhilHealth: TFloatField
      FieldName = 'PhilHealth'
    end
    object paymonthQWTax: TFloatField
      FieldName = 'WTax'
    end
    object paymonthQWTaxPartial: TFloatField
      FieldName = 'WTaxPartial'
    end
    object paymonthQOtherDeduction: TFloatField
      FieldName = 'OtherDeduction'
    end
    object paymonthQnetpay: TFloatField
      FieldName = 'netpay'
    end
    object paymonthQpaycode: TLongWordField
      FieldName = 'paycode'
    end
    object paymonthQwtaxstatus: TStringField
      FieldName = 'wtaxstatus'
      Size = 10
    end
    object paymonthQtaxable: TFloatField
      FieldName = 'taxable'
    end
    object paymonthQtaxablePartial: TFloatField
      FieldName = 'taxablePartial'
    end
    object paymonthQRegOtHrs: TFloatField
      FieldName = 'RegOtHrs'
    end
    object paymonthQSpcOTHrs: TFloatField
      FieldName = 'SpcOTHrs'
    end
    object paymonthQRestOtHrs: TFloatField
      FieldName = 'RestOtHrs'
    end
    object paymonthQRestOtExHrs: TFloatField
      FieldName = 'RestOtExHrs'
    end
    object paymonthQLegOTHrs: TFloatField
      FieldName = 'LegOTHrs'
    end
    object paymonthQLegOtExHrs: TFloatField
      FieldName = 'LegOtExHrs'
    end
    object paymonthQLegRestOtExPay: TFloatField
      FieldName = 'LegRestOtExPay'
    end
    object paymonthQLegRestOtHrs: TFloatField
      FieldName = 'LegRestOtHrs'
    end
    object paymonthQLegRestOtExHrs: TFloatField
      FieldName = 'LegRestOtExHrs'
    end
    object paymonthQSpcOtExHrs: TFloatField
      FieldName = 'SpcOtExHrs'
    end
    object paymonthQNSDHrs: TFloatField
      FieldName = 'NSDHrs'
    end
    object paymonthQUndertimeHrs: TFloatField
      FieldName = 'UndertimeHrs'
    end
    object paymonthQAbsencesDays: TFloatField
      FieldName = 'AbsencesDays'
    end
    object paymonthQPremiumDays: TFloatField
      FieldName = 'PremiumDays'
    end
    object paymonthQTardiHrs: TFloatField
      FieldName = 'TardiHrs'
    end
    object paymonthQDailyRate: TFloatField
      FieldName = 'DailyRate'
    end
    object paymonthQemployeestatus: TStringField
      FieldName = 'employeestatus'
      Size = 1
    end
    object paymonthQDayPresent: TFloatField
      FieldName = 'DayPresent'
    end
    object paymonthQTardi: TFloatField
      FieldName = 'Tardi'
    end
    object paymonthQperMonetize: TFloatField
      FieldName = 'perMonetize'
    end
    object paymonthQperLeaveCredits: TFloatField
      FieldName = 'perLeaveCredits'
    end
    object paymonthQRegOTRate: TFloatField
      FieldName = 'RegOTRate'
    end
    object paymonthQSpcOTRate: TFloatField
      FieldName = 'SpcOTRate'
    end
    object paymonthQSpcExOTRate: TFloatField
      FieldName = 'SpcExOTRate'
    end
    object paymonthQLegOTRate: TFloatField
      FieldName = 'LegOTRate'
    end
    object paymonthQLegExOTRate: TFloatField
      FieldName = 'LegExOTRate'
    end
    object paymonthQLegRestOTRate: TFloatField
      FieldName = 'LegRestOTRate'
    end
    object paymonthQLegRestExOTRate: TFloatField
      FieldName = 'LegRestExOTRate'
    end
    object paymonthQisLock: TByteField
      FieldName = 'isLock'
    end
    object paymonthQDF: TDateTimeField
      FieldName = 'DF'
    end
    object paymonthQDT: TDateTimeField
      FieldName = 'DT'
    end
    object paymonthQlmabod: TFloatField
      FieldName = 'lmabod'
    end
    object paymonthQprorate: TFloatField
      FieldName = 'prorate'
    end
    object paymonthQprerate: TFloatField
      FieldName = 'prerate'
    end
    object paymonthQSpcrestpay: TFloatField
      FieldName = 'Spcrestpay'
    end
    object paymonthQSpcRestHrs: TFloatField
      FieldName = 'SpcRestHrs'
    end
    object paymonthQSpcrestOTrate: TFloatField
      FieldName = 'SpcrestOTrate'
    end
    object paymonthQPAGIBIGsavings: TFloatField
      FieldName = 'PAGIBIGsavings'
    end
    object paymonthQOtherDeductiontmp: TFloatField
      FieldName = 'OtherDeductiontmp'
    end
    object paymonthQnetpaytmp: TFloatField
      FieldName = 'netpaytmp'
    end
  end
  object paymonthDetailsQ: TMyQuery
    Connection = PayrollData.MyConnection1
    SQL.Strings = (
      'Select '
      '  f.*,'
      
        '  Concat('#39'Payroll Month: '#39',DATE_FORMAT(f.paymonth, "%M, %Y")) da' +
        'teF'
      'from ('
      'Select p.*,e.deptcode,e.pagibigno,d.departmentname'
      'from zanecopayroll.pmdeduction p '
      'left join zanecopayroll.employee e on e.empnumber = p.empnumber'
      
        'left join zanecopayroll.department d on d.departmentcode = e.dep' +
        'tcode'
      'where p.description = :descrp and '
      '      p.paymonth = :paymonth  and'
      '      p.paydesc = :paydesc order by p.name '
      ') f'
      'order by f.deptcode,f.name asc')
    Active = True
    Left = 688
    Top = 56
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'descrp'
        Value = nil
      end
      item
        DataType = ftUnknown
        Name = 'paymonth'
        Value = nil
      end
      item
        DataType = ftUnknown
        Name = 'paydesc'
        Value = nil
      end>
    object paymonthDetailsQidpmdeduction: TLongWordField
      AutoGenerateValue = arAutoInc
      FieldName = 'idpmdeduction'
      Origin = 'f.idpmdeduction'
    end
    object paymonthDetailsQidemployee: TFloatField
      FieldName = 'idemployee'
      Origin = 'f.idemployee'
    end
    object paymonthDetailsQPaymonth: TDateField
      FieldName = 'Paymonth'
      Origin = 'f.Paymonth'
    end
    object paymonthDetailsQempnumber: TStringField
      FieldName = 'empnumber'
      Origin = 'f.empnumber'
      Size = 15
    end
    object paymonthDetailsQname: TStringField
      FieldName = 'name'
      Origin = 'f.name'
      Size = 50
    end
    object paymonthDetailsQfixdeductionindex: TLongWordField
      FieldName = 'fixdeductionindex'
      Origin = 'f.fixdeductionindex'
    end
    object paymonthDetailsQloanschedule: TLongWordField
      FieldName = 'loanschedule'
      Origin = 'f.loanschedule'
    end
    object paymonthDetailsQdescription: TStringField
      FieldName = 'description'
      Origin = 'f.description'
      Size = 50
    end
    object paymonthDetailsQamount: TFloatField
      FieldName = 'amount'
      Origin = 'f.amount'
    end
    object paymonthDetailsQemployeecontribution: TFloatField
      FieldName = 'employeecontribution'
      Origin = 'f.employeecontribution'
    end
    object paymonthDetailsQemployercontribution: TFloatField
      FieldName = 'employercontribution'
      Origin = 'f.employercontribution'
    end
    object paymonthDetailsQemployeecompensation: TFloatField
      FieldName = 'employeecompensation'
      Origin = 'f.employeecompensation'
    end
    object paymonthDetailsQpaycode: TStringField
      FieldName = 'paycode'
      Origin = 'f.paycode'
      Size = 1
    end
    object paymonthDetailsQpaydesc: TStringField
      FieldName = 'paydesc'
      Origin = 'f.paydesc'
      Size = 150
    end
    object paymonthDetailsQactualremittance: TFloatField
      FieldName = 'actualremittance'
      Origin = 'f.actualremittance'
    end
    object paymonthDetailsQdeptcode: TStringField
      FieldName = 'deptcode'
      Origin = 'f.deptcode'
      Size = 30
    end
    object paymonthDetailsQpagibigno: TStringField
      FieldName = 'pagibigno'
      Origin = 'f.pagibigno'
      Size = 15
    end
    object paymonthDetailsQdepartmentname: TStringField
      FieldName = 'departmentname'
      Origin = 'f.departmentname'
      Size = 50
    end
    object paymonthDetailsQdateF: TStringField
      FieldName = 'dateF'
      Origin = 'dateF'
      Size = 85
    end
  end
  object descriptionQ: TMyQuery
    Connection = PayrollData.MyConnection1
    SQL.Strings = (
      'Select * from pagibigsavingdesc limit 1')
    Left = 776
    Top = 56
    object descriptionQidpagibigsavingDesc: TIntegerField
      FieldName = 'idpagibigsavingDesc'
    end
    object descriptionQdescription: TStringField
      FieldName = 'description'
      Size = 55
    end
  end
  object paymonthDetailsRep: TfrxReport
    Version = '5.3.16'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 43871.456121296300000000
    ReportOptions.LastChange = 43871.467989919000000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 808
    Top = 113
    Datasets = <
      item
        DataSet = pdetailsDS
        DataSetName = 'pdetailsDS'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 210.000000000000000000
      PaperHeight = 297.000000000000000000
      PaperSize = 9
      LeftMargin = 5.000000000000000000
      RightMargin = 5.000000000000000000
      TopMargin = 5.000000000000000000
      BottomMargin = 5.000000000000000000
      object ReportTitle1: TfrxReportTitle
        FillType = ftBrush
        Height = 102.047310000000000000
        Top = 18.897650000000000000
        Width = 755.906000000000000000
        object Memo1: TfrxMemoView
          Align = baCenter
          Left = 175.748145000000000000
          Top = 7.559060000000000000
          Width = 404.409710000000000000
          Height = 15.118120000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            'ZAMBOANGA DEL NORTE ELECTRIC COOPERATIVE, INC.')
          ParentFont = False
        end
        object Memo2: TfrxMemoView
          Align = baCenter
          Left = 175.748145000000000000
          Top = 25.236240000000000000
          Width = 404.409710000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            'Gen. Luna St., Dipolog City')
          ParentFont = False
        end
        object Memo3: TfrxMemoView
          Align = baCenter
          Left = 175.748145000000000000
          Top = 54.133890000000000000
          Width = 404.409710000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            'MONTHLY PAG-IBIG SAVINGS')
          ParentFont = False
        end
        object pdetailsDSdateF: TfrxMemoView
          Align = baCenter
          Left = 177.637910000000000000
          Top = 71.811070000000000000
          Width = 400.630180000000000000
          Height = 18.897650000000000000
          DataField = 'dateF'
          DataSet = pdetailsDS
          DataSetName = 'pdetailsDS'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[pdetailsDS."dateF"]')
          ParentFont = False
        end
      end
      object DetailData1: TfrxDetailData
        FillType = ftBrush
        Height = 26.456710000000000000
        Top = 275.905690000000000000
        Width = 755.906000000000000000
        DataSet = pdetailsDS
        DataSetName = 'pdetailsDS'
        RowCount = 0
        object pdetailsDSname: TfrxMemoView
          Left = 45.574830000000000000
          Top = 3.779530000000000000
          Width = 268.346630000000000000
          Height = 18.897650000000000000
          StretchMode = smActualHeight
          DataField = 'name'
          DataSet = pdetailsDS
          DataSetName = 'pdetailsDS'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[pdetailsDS."name"]')
          ParentFont = False
        end
        object pdetailsDSpagibigno: TfrxMemoView
          Left = 320.480520000000000000
          Top = 3.779530000000000000
          Width = 143.622140000000000000
          Height = 18.897650000000000000
          DataField = 'pagibigno'
          DataSet = pdetailsDS
          DataSetName = 'pdetailsDS'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[pdetailsDS."pagibigno"]')
          ParentFont = False
        end
        object pdetailsDSamount: TfrxMemoView
          Left = 616.063390000000000000
          Top = 3.779530000000000000
          Width = 109.606370000000000000
          Height = 18.897650000000000000
          DataField = 'amount'
          DataSet = pdetailsDS
          DataSetName = 'pdetailsDS'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[pdetailsDS."amount"]')
          ParentFont = False
        end
        object SysMemo1: TfrxSysMemoView
          Left = 4.559060000000000000
          Top = 3.779530000000000000
          Width = 37.795300000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[LINE#]')
          ParentFont = False
        end
      end
      object GroupHeader1: TfrxGroupHeader
        FillType = ftBrush
        Height = 49.133890000000000000
        Top = 204.094620000000000000
        Width = 755.906000000000000000
        Condition = 'pdetailsDS."deptcode"'
        object pdetailsDSdepartmentname: TfrxMemoView
          Left = 113.385900000000000000
          Width = 226.771800000000000000
          Height = 18.897650000000000000
          DataField = 'departmentname'
          DataSet = pdetailsDS
          DataSetName = 'pdetailsDS'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            '[pdetailsDS."departmentname"]')
          ParentFont = False
        end
        object Memo6: TfrxMemoView
          Left = 3.779530000000000000
          Width = 102.047310000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'DEPARTMENT: ')
          ParentFont = False
        end
        object Memo5: TfrxMemoView
          Left = 3.779530000000000000
          Top = 22.677180000000000000
          Width = 34.015770000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'No.')
          ParentFont = False
        end
        object Memo7: TfrxMemoView
          Left = 41.574830000000000000
          Top = 22.677180000000000000
          Width = 52.913420000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Name')
          ParentFont = False
        end
        object Memo8: TfrxMemoView
          Left = 317.480520000000000000
          Top = 22.677180000000000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Pag-ibig No.')
          ParentFont = False
        end
        object Memo9: TfrxMemoView
          Left = 630.063390000000000000
          Top = 22.677180000000000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'Amount')
          ParentFont = False
        end
        object Line1: TfrxLineView
          Left = 3.779530000000000000
          Top = 45.354360000000000000
          Width = 748.346940000000000000
          Color = clBlack
          Diagonal = True
        end
        object Line2: TfrxLineView
          Left = 3.779530000000000000
          Top = 18.897650000000000000
          Width = 748.346456690000000000
          Color = clBlack
          Diagonal = True
        end
      end
      object GroupFooter1: TfrxGroupFooter
        FillType = ftBrush
        Height = 49.133890000000000000
        Top = 325.039580000000000000
        Width = 755.906000000000000000
        object Memo10: TfrxMemoView
          Left = 7.559060000000000000
          Top = 7.559060000000000000
          Width = 226.771800000000000000
          Height = 18.897650000000000000
          DataField = 'departmentname'
          DataSet = pdetailsDS
          DataSetName = 'pdetailsDS'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            '[pdetailsDS."departmentname"]')
          ParentFont = False
        end
        object Memo11: TfrxMemoView
          Left = 483.779840000000000000
          Top = 7.559060000000000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'Sub Total')
          ParentFont = False
        end
        object SysMemo2: TfrxSysMemoView
          Left = 593.386210000000000000
          Top = 7.559060000000000000
          Width = 132.283550000000000000
          Height = 18.897650000000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<pdetailsDS."amount">,DetailData1)]')
          ParentFont = False
        end
        object Line3: TfrxLineView
          Left = 3.220470000000000000
          Top = 2.000000000000000000
          Width = 748.346456690000000000
          Color = clBlack
          Diagonal = True
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        Top = 181.417440000000000000
        Width = 755.906000000000000000
        DataSet = pdetailsDS
        DataSetName = 'pdetailsDS'
        RowCount = 0
      end
      object ReportSummary1: TfrxReportSummary
        FillType = ftBrush
        Height = 49.133890000000000000
        Top = 434.645950000000000000
        Width = 755.906000000000000000
        object Memo12: TfrxMemoView
          Left = 483.779840000000000000
          Top = 26.559060000000000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'Grand Total')
          ParentFont = False
        end
        object SysMemo3: TfrxSysMemoView
          Left = 593.386210000000000000
          Top = 26.559060000000000000
          Width = 132.283550000000000000
          Height = 18.897650000000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<pdetailsDS."amount">,DetailData1)]')
          ParentFont = False
        end
        object pdetailsDSdescription: TfrxMemoView
          Left = 272.126160000000000000
          Top = 3.779530000000000000
          Width = 306.141930000000000000
          Height = 18.897650000000000000
          DataField = 'description'
          DataSet = pdetailsDS
          DataSetName = 'pdetailsDS'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[pdetailsDS."description"]')
          ParentFont = False
        end
      end
    end
  end
  object pdetailsDS: TfrxDBDataset
    UserName = 'pdetailsDS'
    CloseDataSource = False
    FieldAliases.Strings = (
      'idpmdeduction=idpmdeduction'
      'idemployee=idemployee'
      'Paymonth=Paymonth'
      'empnumber=empnumber'
      'name=name'
      'fixdeductionindex=fixdeductionindex'
      'loanschedule=loanschedule'
      'description=description'
      'amount=amount'
      'employeecontribution=employeecontribution'
      'employercontribution=employercontribution'
      'employeecompensation=employeecompensation'
      'paycode=paycode'
      'paydesc=paydesc'
      'actualremittance=actualremittance'
      'deptcode=deptcode'
      'pagibigno=pagibigno'
      'departmentname=departmentname'
      'dateF=dateF')
    DataSet = paymonthDetailsQ
    BCDToCurrency = False
    Left = 760
    Top = 112
  end
  object QExport4XLS1: TQExport4XLS
    DataSet = exportQ
    About = '(About EMS AdvancedExport)'
    _Version = '4.10.2.1'
    Formats.DecimalSeparator = '.'
    Formats.ThousandSeparator = ','
    Formats.DateSeparator = '/'
    Formats.TimeSeparator = ':'
    Formats.BooleanTrue = 'true'
    Formats.BooleanFalse = 'false'
    Formats.IntegerFormat = '#,###,##0'
    Formats.FloatFormat = '#,###,##0.00'
    Formats.DateFormat = 'MM/dd/yyyy'
    Formats.TimeFormat = 'h:mm AMPM'
    Formats.DateTimeFormat = 'MM/dd/yyyy h:mm:ss AMPM'
    Formats.CurrencyFormat = #8369'#,###,##0.00'
    AutoCalcColWidth = True
    Options.PageFooter = 'Page &P of &N'
    Options.SheetTitle = 'Sheet 1'
    Options.CaptionsFormat.Font.Style = [xfsBold]
    Options.HyperlinkFormat.Font.Color = clrBlue
    Options.HyperlinkFormat.Font.Underline = fulSingle
    Options.NoteFormat.Alignment.Horizontal = halLeft
    Options.NoteFormat.Alignment.Vertical = valTop
    Options.NoteFormat.Font.Size = 8
    Options.NoteFormat.Font.Style = [xfsBold]
    Options.NoteFormat.Font.Name = 'Tahoma'
    FieldFormats = <>
    StripStyles = <>
    Hyperlinks = <>
    Notes = <>
    Charts = <>
    Sheets = <>
    Pictures = <>
    Images = <>
    Cells = <>
    Aggregates = <>
    MergedCells = <>
    DefRowHeight = 12.750000000000000000
    Left = 744
    Top = 176
  end
  object exportQ: TMyQuery
    Connection = PayrollData.MyConnection1
    SQL.Strings = (
      'set @cnt :=0;'
      'Select '
      '  @cnt := @cnt+1 as No,'
      '  f.departmentname as Department,'
      '  f.Paymonth,'
      '  f.Name,'
      '  f.PagibigNo,'
      '  f.Amount'
      '  '
      'from ('
      'Select p.*,e.deptcode,e.pagibigno,d.departmentname'
      'from zanecopayroll.pmdeduction p '
      'left join zanecopayroll.employee e on e.empnumber = p.empnumber'
      
        'left join zanecopayroll.department d on d.departmentcode = e.dep' +
        'tcode'
      'where p.description = :descrp and '
      '      p.paymonth = :paymonth  and'
      '      p.paydesc = :paydesc order by p.name '
      ') f'
      'order by f.deptcode,f.name asc')
    Left = 816
    Top = 168
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'descrp'
        Value = nil
      end
      item
        DataType = ftUnknown
        Name = 'paymonth'
        Value = nil
      end
      item
        DataType = ftUnknown
        Name = 'paydesc'
        Value = nil
      end>
    object exportQNo: TLargeintField
      FieldName = 'No'
    end
    object exportQDepartment: TStringField
      FieldName = 'Department'
      Size = 50
    end
    object exportQPaymonth: TDateField
      FieldName = 'Paymonth'
    end
    object exportQName: TStringField
      FieldName = 'Name'
      Size = 50
    end
    object exportQPagibigNo: TStringField
      FieldName = 'PagibigNo'
      Size = 15
    end
    object exportQAmount: TFloatField
      FieldName = 'Amount'
    end
  end
end
